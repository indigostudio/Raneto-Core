...
var raneto = {
  ...
  normalizeHomeSlug: function(slug) {
      if (slug === '/') {
        return '/index';
      }
  ...
  getLangPrefix: function(slug, useDefaultWhenNone) {
      if (!raneto.config.default_lang_path) {
        return '';
      }
  ...
  processVars: function (markdownContent, slug) {
    ...
    if (typeof raneto.config.image_url !== 'undefined') {
      let base_image_url = raneto.config.base_url + '/' + this.getLangPrefix(slug) + raneto.config.image_url.slice(1);
      markdownContent = markdownContent.replace(/\%image_url\%/g, base_image_url);
    }
  ...
  getPage: function (filePath) {
    try {
      ...
      return {
        ...
        body    : html,
        meta    : meta,
        excerpt : _s.prune(_s.stripTags(_s.unescapeHTML(html.replace(/>/g, '> '))), (raneto.config.excerpt_length || 400))
      };
  ...
  renderPage: function(content, options) {
    options = _.assign({renderer: buildMarkedReader()}, options);
  ...
  getPages: function (activePageSlug) {
    ...
    files.forEach(function (filePath) {
      var shortPath = path.relative(targetDir, filePath).replace(/\\/g, '/');
      ...
      if (stat.isDirectory()) {
        ...
        var sort = 0;
        // ignore directories with an ignore file under it
    ...
    for (let index = 0; index < filesProcessed.length; index++) {
      var category = filesProcessed[index];
      if (category.files.length > 0){
        category.files = _.sortBy(category.files, function (file) { return file.sort; });
        category.isCategoryExpanded = category.files.some(function (file){ return file.active; });
      }
      else if (!category.is_index) {
        // Ignore folders without markdown files
        filesProcessed.splice(index, 1);
        index--;
      }
    }
  ...
  _indexFile(idx, targetDir, filePath) {
    return new Promise(function (indexFileResolve, indexFileReject) {
      // disregard files in directories with an ignore file
      var dir = filePath.substring(0, filePath.lastIndexOf('/'));
      var ignoreFile = dir +'/ignore';
      if (!(fs.existsSync(ignoreFile) && fs.lstatSync(ignoreFile).isFile())) {
        var shortPath = path.relative(targetDir, filePath).replace(/\\/g, '/');
        fs.readFile(filePath, function (err, fileContent) {
          if (err) {
            if (raneto.config.debug) { console.log(err); }
            indexFileResolve();
            return;
          }

          var meta = raneto.processMeta(fileContent.toString('utf-8'));
  ...
  _createIndex: function(slug, targetDir) {
    var id = raneto.getLunrLanguageId(slug);
    if (id !== '') {
        require('lunr-languages/' + id + '.js')(lunr);
    }
    ...
    return new Promise(function(resolve, reject) {
      glob(targetDir + '**/*.md', function(err, files) {
        if (err) {
          if (raneto.config.debug) { console.log(err); }
          resolve(idx);
          return;
        ...
        let indexPromises = files.map(function (filePath) {
          return raneto._indexFile(idx, targetDir, filePath);
        });

        Promise
          .all(indexPromises)
          .catch(function(e) { if (raneto.config.debug) { console.log(e); }})
          .then(function() { resolve(idx); });
      });
    });
  },
  ...
  doSearch: function(slug, query) {
    ...
    return raneto._createIndex(slug, targetDir).then(function(idx) {
